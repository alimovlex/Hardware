...
RST_CLK_PCLKcmd(RST_CLK_PCLK_PORTD, ENABLE); // ????????? ???????????? ????? D
	...
	PORT_Init(MDR_PORTD, &PORTDInit); // ????????????? ????? D ????? ????????? ??????????
	...
	PORT_Init(MDR_PORTC, &PORTEInit); // ????????????? ????? D ????? ????????? ??????????

PORT_SetBits(MDR_PORTD, PORT_Pin_10); // ????????? ??????? ?? 10 ???? ????? D	
		delay(0xFFFF); // ????????
		PORT_ResetBits(MDR_PORTD, PORT_Pin_10); // ????????? ???? ?? 10 ??? ????? D
		...
		PORT_SetBits(MDR_PORTD, PORT_Pin_14); // ????????? ??????? ?? 10 ???? ????? D	
		delay(0xFFFF); // ????????
		PORT_ResetBits(MDR_PORTD, PORT_Pin_14); // ????????? ???? ?? 10 ??? ????? D
		PORT_SetBits(MDR_PORTD, PORT_Pin_13);
		delay(0xFFFF);
		PORT_ResetBits(MDR_PORTD, PORT_Pin_13);
		PORT_SetBits(MDR_PORTD, PORT_Pin_12);
